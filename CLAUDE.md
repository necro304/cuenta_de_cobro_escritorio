...
No hay tests configurados. CI (PRs a `main`) corre: lint, `vue-tsc --noEmit` y `npx vite build`. El trabajo diario va en la rama `dev`; los PRs apuntan a `main`. Los releases se publican con tags `v*` (matriz Linux/Windows/macOS hacia GitHub Releases del repo `necro304/cuenta_de_cobro_escritorio`, usado también por electron-updater para auto-actualización).

## Arquitectura

### Proceso main (`electron/`)
- `main.ts` — ventana, handlers IPC y electron-updater. Los handlers IPC son **genéricos de SQL**, no por entidad: `db-query` (all), `db-get` (get), `db-run` (run), más `db-backup`/`db-restore` (diálogos de archivo; restore reinicia la app) y `check-for-updates`.
