...
<template>
  <div class="space-y-12">
    ...
    >
      ...
      <div>
        ...
        <div class="flex items-center gap-3">
          ...
          <p
            class="font-mono text-xs font-bold tracking-widest text-muted-foreground uppercase flex items-center"
          >
      ...
      <div
        class="hidden md:flex w-20 h-20 bg-accent border-[3px] border-foreground shadow-[4px_4px_0_0_hsl(var(--foreground))] items-center justify-center relative overflow-hidden"
      >
        <div
    ...
    <div class="grid gap-8 md:grid-cols-3">
      <!-- Stat Card 1 -->
      <div
        class="border-[3px] border-foreground p-6 bg-card relative group transition-all hover:-translate-y-2 hover:-translate-x-2 hover:shadow-[12px_12px_0_0_hsl(var(--accent))] shadow-[6px_6px_0_0_hsl(var(--foreground))] overflow-hidden"
      >
        <DataWaves />
        <div
          class="absolute -right-6 -top-6 w-24 h-24 bg-secondary rounded-full -z-10 group-hover:bg-accent/20 transition-colors"
        ></div>
        <div
          class="absolute top-0 right-0 px-2 py-1 bg-foreground text-background font-mono text-[0.65rem] font-bold border-b-[3px] border-l-[3px] border-foreground"
        >
          STAT_01
        </div>
        ...
        <div>
          <div class="text-6xl font-black tracking-tighter" v-if="!isLoading">
            {{ stats.totalInvoices }}
          </div>
      ...
      <!-- Stat Card 3 -->
      <div
        class="border-[3px] border-foreground p-6 bg-accent text-white relative group transition-all hover:-translate-y-2 hover:-translate-x-2 hover:shadow-[12px_12px_0_0_hsl(var(--foreground))] shadow-[6px_6px_0_0_hsl(var(--foreground))] overflow-hidden"
      >
        <DataWaves />
        <div class="absolute -right-6 -top-6 w-24 h-24 bg-white/20 rounded-full -z-10"></div>
        <div
          class="absolute top-0 right-0 px-2 py-1 bg-white text-accent font-mono text-[0.65rem] font-bold border-b-[3px] border-l-[3px] border-foreground"
        >
          STAT_03
        </div>
    ...
    <!-- Charts Grid -->
    <div
      class="grid gap-8 md:grid-cols-3 mt-12"
      v-if="!isLoading && (statusData.length > 0 || monthlyData.length > 0)"
    >
      <!-- Status Doughnut Chart -->
      <div
        class="md:col-span-1 border-[3px] border-foreground p-6 bg-card relative shadow-[6px_6px_0_0_hsl(var(--foreground))] h-[400px] flex flex-col"
      >
        <div
          class="absolute top-0 right-0 px-2 py-1 bg-foreground text-background font-mono text-[0.65rem] font-bold border-b-[3px] border-l-[3px] border-foreground"
        >
      ...
      <div
        class="md:col-span-2 border-[3px] border-foreground p-6 bg-card relative shadow-[6px_6px_0_0_hsl(var(--foreground))] h-[400px] flex flex-col"
      >
        <div
          class="absolute top-0 right-0 px-2 py-1 bg-foreground text-background font-mono text-[0.65rem] font-bold border-b-[3px] border-l-[3px] border-foreground"
        >
        ...
        <h3 class="text-xl font-bold uppercase tracking-wider mb-6">
          Proyección de Ingresos (Facturación)
        </h3>
        <div class="flex-1 relative min-h-0">
    ...
    <div
      class="border-[3px] border-foreground p-8 bg-secondary relative mt-16 shadow-[4px_4px_0_0_hsl(var(--foreground))]"
    >
      <div class="flex flex-col md:flex-row gap-6 items-center justify-between">
        <div>
          <h4 class="text-2xl font-black uppercase tracking-tighter mb-2">Acceso Rápido</h4>
          <p class="font-mono text-sm text-muted-foreground">
            Utilice los comandos para generar nuevas cuentas o gestionar clientes.
          </p>
        </div>
        <div class="flex gap-4 w-full md:w-auto">
          <button
            @click="$router.push('/invoices/new')"
            class="flex-1 md:flex-none bg-foreground text-background font-bold px-6 py-4 uppercase tracking-wider border-[3px] border-foreground hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[6px_6px_0_0_hsl(var(--accent))] transition-all whitespace-nowrap"
          >
            + Nueva Cuenta
          </button>
        </div>
      ...
      <div
        class="absolute bottom-0 left-0 w-full overflow-hidden pointer-events-none opacity-[0.03]"
      >
